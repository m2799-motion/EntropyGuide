...
class UniMoS:     
    ...
    def cal_l(self):
        ...
        step = (start - end) / min(10, total_epochs)  # decay over 10 epochs or fewer
        res = torch.arange(start, end - 0.01, -step)
        extra = torch.ones(total_epochs - len(res)) * end
        return torch.concat([res, extra], 0)
        ...
        for self.epoch in range(0, self.args.epochs):
            ...
            for i, _ in enumerate(tqdm(range(self.epoch_len), mininterval=0.5)):
                ...
                with autocast():
                    ...
                    soft_tar_vis = F.softmax(tar_vis_out, dim=1)
                    soft_tar_txt = F.softmax(tar_txt_out, dim=1)
                    entropy_vis = -torch.sum(soft_tar_vis * torch.log(soft_tar_vis + 1e-5), dim=1)
                    entropy_txt = -torch.sum(soft_tar_txt * torch.log(soft_tar_txt + 1e-5), dim=1)
                    conf_vis = 1 - entropy_vis / torch.log(torch.tensor(soft_tar_vis.size(1), device=soft_tar_vis.device))
                    conf_txt = 1 - entropy_txt / torch.log(torch.tensor(soft_tar_txt.size(1), device=soft_tar_txt.device))
                    w = conf_vis / (conf_vis + conf_txt + 1e-6)
                    w = w.view(-1, 1)
                    ens_tar_vis_out = w * tar_vis_out + (1 - w) * tar_txt_out.detach()
    ...
    def test(self, d='tar'):
        ...
        # pasting code from chat here
        if acc_mix >= self.best['acc']:
            save_path = os.path.join(self.config.OUTPUT, f"best_model_epoch{self.epoch}.pth")
            torch.save({
                'bottleneck': self.bottleneck.state_dict(),
                'classifier': self.clf.state_dict(),
                'epoch': self.epoch
            }, save_path)
            self.logger.info(f"Saved new best model to {save_path}")
        # code from chat ends here
